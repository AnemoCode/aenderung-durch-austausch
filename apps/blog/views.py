...
from .forms import CommentForm, TopicPartCreateForm, TopicPartEditForm
...
class PostDetailRedirectView(RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        topic = get_object_or_404(Topic, legacy_post_id=kwargs["pk"], is_published=True)
...
class TopicPartUpdateView(LoginRequiredMixin, FormView):
    template_name = "blog/post_edit_form.html"
    form_class = TopicPartEditForm
    login_url = reverse_lazy("accounts:login")

    def dispatch(self, request, *args, **kwargs):
        if request.user.is_authenticated:
            if not (
                request.user.is_staff or request.user.has_perm("blog.change_topicpart")
            ):
                return redirect("blog:index")
            self.object = get_object_or_404(TopicPart, pk=kwargs["pk"])
        return super().dispatch(request, *args, **kwargs)
    ...
    def get_initial(self):
        ...
        return {
            ...
            "body": part.body,
            "tags": ", ".join(t.name for t in part.tags.all()),
        }

    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx["active_nav"] = "index"
        ctx["part"] = self.object
        return ctx

    def form_valid(self, form):
        part = self.object
        part.heading = form.cleaned_data["heading"]
        part.body = form.cleaned_data["body"]
        part.save()
        part.tags.set(form.parsed_tags())
